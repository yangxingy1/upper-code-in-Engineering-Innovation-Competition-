...
class Detection(object):
    ...
    def detect_colored_circle(self, color:str, min_r=100) -> bool:
        if color == "red":
            lower_color = self.lower_red
            upper_color = self.upper_red
        elif color == "green":
        ...
        while True:
            ...
            x2, y2 = x + r1, y + r1
            # 内接正方形
            cutted_frame = img[y1:y2, x1:x2].copy()
            if cutted_frame is None or cutted_frame.size == 0 or cutted_frame.ndim != 3:
                print('error')
                continue
            hsv_frame = cv.cvtColor(cutted_frame, cv.COLOR_RGB2HSV)
            # 外接正方形
            cut_frame = img[y10:y20, x10:x20].copy()
            #cv.imshow("c",cut_frame)
            #cv.waitKey()
            #cv.destroyAllWindows()
